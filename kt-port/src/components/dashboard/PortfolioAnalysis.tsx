'use client';

import { useState, useEffect } from 'react';
import ChartWrapper from '@/components/ui/ChartWrapper';

interface MonthlyData {
  ReportingMonth: string;
  Revenue: number;
  Expenses: number;
  NOI: number;
  CashFlow: number;
  Vacancy: number;
}

export default function PortfolioAnalysis() {
  const [monthlyData, setMonthlyData] = useState<MonthlyData[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    fetchMonthlyData();
  }, []);

  const fetchMonthlyData = async () => {
    try {
      setLoading(true);
      const year = new Date().getFullYear();
      const response = await fetch(`/api/dashboard/monthly-performance?year=${year}`);
      
      if (!response.ok) {
        throw new Error('Failed to fetch monthly data');
      }
      
      const data = await response.json();
      setMonthlyData(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load data');
    } finally {
      setLoading(false);
    }
  };

  if (loading) {
    return (
      <div className="d-flex justify-content-center py-5">
        <div className="spinner-border text-primary" role="status">
          <span className="visually-hidden">Loading...</span>
        </div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="alert alert-danger" role="alert">
        <h4 className="alert-heading">Error Loading Charts</h4>
        <p>{error}</p>
        <button className="btn btn-outline-danger" onClick={fetchMonthlyData}>
          Try Again
        </button>
      </div>
    );
  }

  // Prepare chart data
  const months = monthlyData.map(d => {
    const date = new Date(d.ReportingMonth);
    return date.toLocaleDateString('en-US', { month: 'short' });
  });

  const revenueExpensesData = {
    labels: months,
    datasets: [
      {
        label: 'Revenue',
        data: monthlyData.map(d => d.Revenue),
        borderColor: '#0C223A',
        backgroundColor: 'rgba(12, 34, 58, 0.1)',
        borderWidth: 3,
        tension: 0.1,
      },
      {
        label: 'Expenses',
        data: monthlyData.map(d => d.Expenses),
        borderColor: '#87CEEB',
        backgroundColor: 'rgba(135, 206, 235, 0.1)',
        borderWidth: 3,
        tension: 0.1,
      },
    ],
  };

  const noiCashFlowData = {
    labels: months,
    datasets: [
      {
        label: 'NOI',
        data: monthlyData.map(d => d.NOI),
        backgroundColor: '#0C223A',
        type: 'bar' as const,
      },
      {
        label: 'Cash Flow',
        data: monthlyData.map(d => d.CashFlow),
        borderColor: '#87CEEB',
        backgroundColor: 'rgba(135, 206, 235, 0.2)',
        borderWidth: 2,
        type: 'line' as const,
        tension: 0.1,
      },
    ],
  };

  const expenseBreakdownData = {
    labels: ['Repairs', 'Utilities', 'Management', 'Taxes', 'Insurance'],
    datasets: [
      {
        data: [25, 15, 20, 25, 15],
        backgroundColor: [
          '#0C223A',
          '#87CEEB',
          '#4682B4',
          '#5F9EA0',
          '#B0E0E6',
        ],
        borderWidth: 0,
      },
    ],
  };

  const chartOptions = {
    plugins: {
      title: {
        display: true,
        font: {
          size: 16,
          weight: 'bold',
        },
        color: '#0C223A',
      },
      legend: {
        position: 'top' as const,
      },
    },
    scales: {
      y: {
        beginAtZero: true,
        ticks: {
          callback: function(value: any) {
            return '$' + value.toLocaleString();
          },
        },
      },
    },
  };

  return (
    <div className="content-card">
      <h4 className="mb-4">📊 Monthly Performance Trends</h4>
      
      {monthlyData.length === 0 ? (
        <div className="alert alert-info" role="alert">
          <h5 className="alert-heading">No Data Available</h5>
          <p>No monthly performance data found for the current year. Add some financial data to see charts.</p>
        </div>
      ) : (
        <>
          {/* Revenue vs Expenses Chart */}
          <div className="mb-4">
            <ChartWrapper
              type="line"
              data={revenueExpensesData}
              options={{
                ...chartOptions,
                plugins: {
                  ...chartOptions.plugins,
                  title: {
                    ...chartOptions.plugins.title,
                    text: 'Revenue vs Expenses',
                  },
                },
              }}
              height={400}
            />
          </div>

          {/* NOI and Cash Flow Charts */}
          <div className="row g-4 mt-2">
            <div className="col-lg-6">
              <div className="content-card">
                <ChartWrapper
                  type="bar"
                  data={noiCashFlowData}
                  options={{
                    ...chartOptions,
                    plugins: {
                      ...chartOptions.plugins,
                      title: {
                        ...chartOptions.plugins.title,
                        text: 'NOI & Cash Flow',
                      },
                    },
                  }}
                  height={300}
                />
              </div>
            </div>
            <div className="col-lg-6">
              <div className="content-card">
                <ChartWrapper
                  type="doughnut"
                  data={expenseBreakdownData}
                  options={{
                    plugins: {
                      title: {
                        display: true,
                        text: 'Expense Breakdown',
                        font: {
                          size: 16,
                          weight: 'bold',
                        },
                        color: '#0C223A',
                      },
                      legend: {
                        position: 'bottom' as const,
                      },
                    },
                  }}
                  height={300}
                />
              </div>
            </div>
          </div>
        </>
      )}
    </div>
  );
}